...
class HDMapNetDataset(Dataset):
    def __init__(self, version, dataroot, data_conf, is_train, lidar_cut_x=False, TOP_X_MIN=-20, TOP_X_MAX=20):
    ...
    def get_lidar(self, rec):
        ...
        if self.lidar_cut_x:
            idx = np.where(lidar_data[:, 0] > self.TOP_X_MIN)
            lidar_data = lidar_data[idx]
            idx = np.where(lidar_data[:, 0] < self.TOP_X_MAX)
            lidar_data = lidar_data[idx]
...
class HDMapNetSemanticDataset(HDMapNetDataset):
    def __init__(self, version, dataroot, data_conf, is_train, lidar_cut_x=False):
        super(HDMapNetSemanticDataset, self).__init__(
            version, dataroot, data_conf, is_train, lidar_cut_x)
...
def semantic_dataset(version, dataroot, data_conf, bsz, nworkers, lidar_cut_x=False):
    train_dataset = HDMapNetSemanticDataset(
        version, dataroot, data_conf, is_train=True, lidar_cut_x=lidar_cut_x)
    val_dataset = HDMapNetSemanticDataset(
        version, dataroot, data_conf, is_train=False, lidar_cut_x=lidar_cut_x)
