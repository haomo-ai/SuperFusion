...
def line_matching_by_CD(inst_pred_lines, inst_pred_confidence, inst_label_lines, seg_pred_lines, seg_label_lines, threshold, bidirectional=False, threshold_iou=0.1):
    ...
    for i, key_pred in enumerate(inst_pred_lines_keys):
        for j, key_label in enumerate(inst_label_lines_keys):
            ...
            seg_pred = torch.zeros(200,200)
            seg_label = torch.zeros(200,200)
...
def instance_mask_AP(AP_matrix, AP_count_matrix, inst_pred_mask, inst_label_mask, scale_x, scale_y, confidence, thresholds, sampled_recalls, bidirectional=False, threshold_iou=0.1):
    # inst_pred: N, C, H, W
    # inst_label: N, C, H, W
    ...
    for n in range(N):
        for c in range(C):
            ...
            seg_label_lines = get_line_seg_from_mask(inst_label_mask[n, c])

            if len(inst_pred_lines) == 0 and len(inst_label_lines) == 0:
                continue
            AP_matrix[c] += single_instance_line_AP(inst_pred_lines, confidence[n], inst_label_lines, thresholds, seg_pred_lines, seg_label_lines, sampled_recalls=sampled_recalls, bidirectional=bidirectional, threshold_iou=threshold_iou)
            AP_count_matrix[c] += 1


def single_instance_line_AP(inst_pred_lines, inst_pred_confidence, inst_label_lines, thresholds, seg_pred_lines, seg_label_lines, sampled_recalls, bidirectional=False, threshold_iou=0.1):
    # inst_pred_line: a list of points {1: (M1, 2), 2: (M2, 2), ..., k2: (M_k2, 2)}
    # inst_pred_confidence: a list of confidence [c1, c2, ..., ck2]
    # inst_label_line: a list of points {1: (N1, 2), 2: (N2, 2), ..., k1: (N_k1, 2)}
    ...
    for t in range(num_thres):
        matching_list = line_matching_by_CD(inst_pred_lines, inst_pred_confidence, inst_label_lines, seg_pred_lines, seg_label_lines, thresholds[t], bidirectional=bidirectional, threshold_iou=threshold_iou)
